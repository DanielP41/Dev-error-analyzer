 ...
 */
export async function openGithubIssue({ errorText, analysis, channel, slackTs }) {
  const title = `[Bot] Error sin resolver detectado en Slack`;
  const slackLink = `https://slack.com/archives/${channel}/p${slackTs.replace(".", "")}`;

  const body = `
## Error detectado automáticamente

**Detectado en Slack:** [Ver mensaje](${slackLink})
